...
public class LoginMenuController {
    ...
    public Result login(String username, String password) {
        ...
        if (username.isEmpty()) return new Result("Please fill in username field.", false);

        if (password.isEmpty()) return new Result("Please fill in password field.", false);

        if (user == null) return new Result("Account not found.\nPlease sign up first.", false);
    ...
    public Result recoverPassword(String username) {
        if (username.isEmpty()) return new Result("Please fill in username field.", false);
        User user = App.findUserByUsername(username);
        if (user == null) return new Result("Username " + username + " not found.", false);
